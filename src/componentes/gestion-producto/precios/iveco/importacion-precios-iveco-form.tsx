...
import { FormValues, crearSchemaValidacion } from "./interfaces-validaciones-precio-iveco";
...
export default function ImportacionPreciosIvecoForm({
  onClose,
  onSuccess,
}: {
  onClose: () => void;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const isValid = await methods.trigger([`cotizacionDolar`]);
      if (!isValid) {
      ...
      await ImportacionPreciosService.importarPreciosIveco(file, payload);
    ...
    <Card className="w-full bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm rounded-3xl overflow-hidden border border-white/20">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 relative">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <CardTitle className="text-xl font-bold text-white">Importar Productos IVECO</CardTitle>
